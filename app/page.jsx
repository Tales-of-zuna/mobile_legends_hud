...
const Home = () => {
    ...
    async function getBattleDataRecursive (payload) {
      ...
      try{
        ...
        data.data.incre_event_list.map(item => {
          if (item.event_type == "kill_hero" && item.extra_param && item.extra_param[0] == "first_blood"){
            // const array = [];
            // array.push(item);
            // setData(item);
            // setType("FirstBlood");
  ...
  useEffect(() => {
    let payload = { authkey: '6646f93ab8cf795f3f78a7ed73469cf7', battleid: '165489743059134510',  dataid: 0};
